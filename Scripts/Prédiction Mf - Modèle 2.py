...
# Script XGBoost (Modèle 2) pour prédire la masse finale de fruits amazoniens à partir de leurs caractéristiques initiales.
# Cette version est adaptée pour prédire le rapport masse initiale / masse finale, utilisé ensuite pour reconstituer la masse finale. Ce choix permet de réduire l'erreur relative de prédiction, en particulier pour les échantillons de petite masse.
# Étapes incluses :
...
arbre_path_base = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2"
...
# Nouvelle cible : facteur de réduction de masse (objectif = minimiser ce rapport masse initiale / masse finale)
df_merged["Facteur_reduction"] = df_merged["Masse_tot_g_initiale"] / df_merged["Masse_tot_g_finale"]
...
def repeated_stratified_kfold_optuna(X, strat_col, n_splits=4, n_repeats=1, base_seed=42):
...
def objective(trial):
    ...
    for train_idx, test_idx in repeated_stratified_kfold_optuna(X_train, strat_col_train, n_splits=4, n_repeats=1):
...
optuna_history_path = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2/optuna_evolution_objective.png"
...
for fold_number, (train_index, test_index) in enumerate(
    ...
    # Conversion du facteur prédit en masse finale prédite pour évaluer la performance réelle
...
# Conversion du facteur en masse finale prédite
y_pred_factor = final_model.predict(X_test)

# Conversion en masse finale prédite
...
# Bootstrap sur le jeu de test (pour évaluer les IC95% sur masse finale prédite)
for i in range(n_iterations):
    ...
    # Calcul des métriques (comparaison entre masses finales observées et prédites)
...
plt.grid(True)
plt.tight_layout()
cv_evolution_path = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2/evolution_cv_scores.png"
...
evolution_mae_path = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2/evolution_cv_MAE.png"
plt.savefig(evolution_mae_path)
plt.close()

# Sauvegarde du modèle pour de futures usages
modele_path = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2/xgboost_Mf_modele_2.joblib"
...
# Conversion du facteur prédit en masse finale prédite
...
df_train_pred["Masse_finale_réelle"] = y_train
...
df_test_pred["Masse_finale_réelle"] = y_test
...
plt.tight_layout()
importance_path = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2/importance_variables.png"
...
for origine, couleur in zip(["Train", "Test"], ["dodgerblue", "darkorange"]):
    ...
    plt.errorbar(
        ...
        yerr=[subset["Masse_finale_prédite_boot"] - subset["IC_inf"],
...
# Sauvegarde de la figure
gain_cumule_path = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2/gain_cumule_par_arbre.png"
plt.savefig(gain_cumule_path)
plt.close()

# Export du fichier Excel avec les différentes feuilles contenant les données
export_path = "D:/Mémoire ULB 2024-2025/1 - Avancées Travail Personnel/Machine Learning Final/XGBoost Mf V2/Modèle 2/Resultats_XGBoost_Mf_Modele2_CV.xlsx"
